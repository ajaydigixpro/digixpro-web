...
export default function KnowledgePage() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#0A0A0A] font-sans selection:bg-[#009E73]/20 pb-16">
      
      {/* 1. HERO SECTION */}
      <section className="max-w-[1200px] mx-auto px-6 pt-20 pb-16 border-b border-neutral-200">
        <div className="max-w-4xl">
          ...
          <p className="text-[18px] md:text-[22px] font-medium text-neutral-600 leading-relaxed max-w-3xl mb-8">
            Real lessons from production systems, consulting engagements, and architecture decisions. 
          </p>

          {/* Philosophy Block */}
          <div className="bg-neutral-50 border-l-4 border-[#009E73] p-6 rounded-r-2xl max-w-3xl">
            <p className="text-[16px] text-neutral-700 font-medium">
              <strong className="text-black">Our Editorial Rule:</strong> Every insight published here originates from a real architecture decision, an advisory engagement, or a live production system. We don't write trend pieces; we document engineering reality to help founders make better technical decisions.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* 2. THE 5 PILLARS */}
      <section className="bg-neutral-50 py-12 border-b border-neutral-200 overflow-hidden">
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 overflow-x-auto">
            {pillars.map((pillar, idx) => (
              <div key={idx} className="flex items-center space-x-3 w-full md:w-auto min-w-max bg-white border border-neutral-200 px-6 py-4 rounded-xl shadow-sm">
                <div className="text-[#009E73]">{pillar.icon}</div>
                <span className="font-bold text-[15px] text-black">{pillar.name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 max-w-[1200px] mx-auto px-6">
        <div className="mb-16">
          <h2 className="text-[32px] md:text-[40px] font-extrabold mb-4">Latest Insights</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          {knowledgeArticles.map((article, idx) => (
            <Link 
              ...
              key={idx}
              className="group block border-b md:border-b-0 md:border border-neutral-200 pb-8 md:p-8 md:rounded-[24px] hover:border-black/30 transition-all bg-white"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-[12px] font-mono font-bold uppercase tracking-wider text-neutral-500 border border-neutral-200 px-3 py-1 rounded-md bg-neutral-50 group-hover:bg-white transition-colors">
                  {article.pillar}
                </span>
              ...
              <h3 className="text-[22px] md:text-[26px] font-extrabold text-black mb-4 leading-snug group-hover:text-[#009E73] transition-colors">
                {article.title}
              </h3>
              ...
              <div className="inline-flex items-center text-[15px] font-bold text-black group-hover:text-[#009E73] transition-colors">
